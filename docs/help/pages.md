...
- 「編集」を押すと、役者を編集できます。
    - 編集画面から役者を削除できます。
- 公演の所有権または編集権があるか、この役者に関連付けられたユーザなら、稽古ごとの参加時間を追加/編集できます。
...
先に「出席率グラフ」を見ると分かりますが、一回の稽古を参加者の出入りによって分割しています。
表示している数値は、この分割された時間帯ごとの以下の式の値の合計です。

式 = 時間帯の長さ (分) × 出席率 ÷ シーンの長さ

### 出席率の基準

「出席率の基準」によって、上の式の「出席率」が変わります。
...
「役者の詳細」で変更できる参加時間を、いつ、誰が変更したかを表示します。
